...
abstract class BasePopup<T : BasePopup<T>> : PopupWindow.OnDismissListener {
        ...
        contentView!!.viewTreeObserver.addOnGlobalLayoutListener(object : OnGlobalLayoutListener {
            override fun onGlobalLayout() {
                ...
                if (mOnRealWHAlreadyListener != null) {
                    mOnRealWHAlreadyListener!!.onRealWHAlready(this@BasePopup, width, height,
                ...
                if (isShowing && isAtAnchorViewMethod) {
                    updateLocation(width, height, mAnchorView, yGravity, xGravity, offsetX, offsetY)
                }
            }
        })
    }
     ...
     *
     * @param focusAndOutsideEnable boolean
     * @return  T
     */
     ...
     * 是否需要重新获取宽高
     *
     * @param needReMeasureWH   needReMeasureWH
     * @return  T
     */
    ...
    fun showAsDropDown(anchor: View?, offsetX: Int, offsetY: Int, gravity: Int) {
        ...
        PopupWindowCompat.showAsDropDown(popupWindow!!, anchor!!, this.offsetX, this.offsetY, gravity)
    }
     ...
     *
     *
     * 使用此方法需要在创建的时候调用setAnchorView等属性设置setAnchorView
     *
     *
     ...
     *
     * @param anchor    View
     * @param vertGravity   vertGravity
     * @param horizGravity  horizGravity
     */
    @JvmOverloads
